...
try {
  ...
  const invoke = async (name, args, { validate = true } = {}) => {
  ...
  const call = async (name, args, options = {}) => {
    let result = await invoke(name, args, options)
    if (name !== 'command_execute' || result.isError === true || !result.structuredContent?.process) return result
    let processState = result.structuredContent.process
    let stdoutCursor = processState.stdout?.next_cursor ?? 0
    let stderrCursor = processState.stderr?.next_cursor ?? 0
    const deadline = Date.now() + 60_000
    while (processState.status === 'running' && Date.now() < deadline) {
      await new Promise((resolve) => setTimeout(resolve, 50))
      const polled = await invoke('command_process_poll', {
        context_id: args.context_id,
        process_id: processState.process_id,
        stdout_cursor: stdoutCursor,
        stderr_cursor: stderrCursor,
      }, options)
      if (polled.isError === true) return polled
      processState = polled.structuredContent?.process
      if (!processState) throw new Error('command_process_poll returned no process state')
      stdoutCursor = processState.stdout?.next_cursor ?? stdoutCursor
      stderrCursor = processState.stderr?.next_cursor ?? stderrCursor
    }
    if (processState.status === 'running') throw new Error(`command process did not finish within black-box timeout: ${processState.process_id}`)
    return await invoke('command_process_result', { context_id: args.context_id, process_id: processState.process_id }, options)
  }
  ...
  if (attachedTarget) {
    const outsideResult = commandResult(outside)
    const outsideDenied = outside.isError === true
      ? ['shell_path_not_allowed', 'filesystem_read_denied'].includes(errorCode(outside))
      : (outsideResult?.return_code ?? 0) !== 0
        && /(?:Operation not permitted|Permission denied|EPERM|EACCES)/i.test(outsideResult?.stderr ?? '')
    check(
  ...
  const concurrentCommand = async (index) => {
    const deadline = Date.now() + 30_000
    for (;;) {
      const result = await call('command_execute', { context_id: contextId, command: `echo AGENT_HELM_MCP_BLACKBOX_PARALLEL_${index}`, purpose: `Verify concurrent MCP command execution ${index}` })
      if (result.isError !== true || errorCode(result) !== 'execution_capacity_unavailable' || Date.now() >= deadline) return result
      await new Promise((resolve) => setTimeout(resolve, 25))
    }
  }
  const concurrentResults = await Promise.all(Array.from({ length: 8 }, (_, index) => concurrentCommand(index)))
  check('concurrent MCP command calls all complete with their own results', concurrentResults.every((result, index) => result.isError !== true && commandResult(result)?.return_code === 0 && commandResult(result)?.stdout?.trim() === `AGENT_HELM_MCP_BLACKBOX_PARALLEL_${index}`), concurrentResults.map((result, index) => `${index}:${resultDetail(result)}`).join(' | '))
